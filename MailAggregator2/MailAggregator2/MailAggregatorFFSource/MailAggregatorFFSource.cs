            ...
            try
            {
                if (inputFilePath.Length > 0 && System.IO.Directory.Exists(inputFilePath))
                {
                    foreach (string fileName in System.IO.Directory.GetFiles(inputFilePath, inputFileMask))
                    {
                        ...
                        if (lines.Length == 0)
                            RaiseAggregatorWarning(string.Format("The file {0} is empty!", fileName));
                        else
                        {
                        ...
                        {
                            if (renex.Message.Contains("The process cannot access the file"))
                                RaiseAggregatorWarning(string.Format("The file '{0}' is still locked (being updated by another process). Will retry next time.", fileName));
                            else
